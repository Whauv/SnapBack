"""Transcription runtime orchestration and delivery."""

# ruff: noqa: D102, D107, PERF203, PLC0415, S603, SIM117, TC003, TC006, TRY300, TRY400
...
TRANSCRIPT_ENDPOINT = "/session/transcript"
MESSAGE_TYPE_SESSION_BEGINS = "SessionBegins"
MESSAGE_TYPE_FINAL_TRANSCRIPT = "FinalTranscript"
MESSAGE_TYPE_ERROR = "Error"
LOCAL_AUDIO_FILENAME = "c.wav"
BACKEND_RETRYABLE_STATUS_CODES = (429, 500, 502, 503, 504)
...
class TranscriptionRuntime:
    ...
    def _build_session(self) -> requests.Session:
        ...
        retries = Retry(
            total=self.config.request_retry_count,
            read=self.config.request_retry_count,
            connect=self.config.request_retry_count,
            backoff_factor=1.0,
            status_forcelist=BACKEND_RETRYABLE_STATUS_CODES,
    ...
    def _on_m(self, _ws: object, msg: str) -> None:
        payload = self._parse_cloud_message(msg)
        if payload is None:
            logger.warning("Ignoring invalid cloud message payload")
            return
        self._handle_cloud_message(payload)

    def _parse_cloud_message(self, msg: str) -> dict[str, object] | None:
        try:
            payload = json.loads(msg)
        except json.JSONDecodeError as e:
            logger.warning("Ignoring non-JSON cloud message: %s", e)
            payload = None
        if not isinstance(payload, dict):
            logger.warning("Ignoring non-object cloud message")
            payload = None
        return payload

    def _handle_cloud_message(self, payload: dict[str, object]) -> None:
        message_type = str(payload.get("message_type", ""))
        if message_type == MESSAGE_TYPE_SESSION_BEGINS:
            self._events["connected"].set()
            return
        if message_type == MESSAGE_TYPE_FINAL_TRANSCRIPT:
            text = payload.get("text")
            if isinstance(text, str) and text.strip():
                self._handle_final_transcript(text)
            return
        if message_type == MESSAGE_TYPE_ERROR:
            logger.error("Cloud error: %s", payload)
    ...
    def _post(self, text: str) -> None:
        ...
        payload = {"session_id": sid, "text": text.strip(), "timestamp": utc_now_iso()}
        url = f"{self.config.backend_base_url}{TRANSCRIPT_ENDPOINT}"
        for attempt in range(self.config.request_retry_count + 1):
            try:
        ...
        with closing(pyaudio.PyAudio()) as pa:
            with closing(
                ...
                while not self._events["stop"].is_set():
                    ...
                    while (
                        time.monotonic() - start < self.config.local_segment_seconds
                    ...
                    if frames:
                        self._tx_local(list(frames))

    def _tx_local(self, frames: list[bytes]) -> None:
        self._submit_local_tx(self._process_local_frames(frames))

    def _process_local_frames(self, frames: list[bytes]) -> str:
        if not frames:
            return ""
        return self._transcribe_local_frames(frames)

    def _submit_local_tx(self, text: str) -> None:
        if text:
            self._post(text)

    def _transcribe_local_frames(self, frames: list[bytes]) -> str:
        with tempfile.TemporaryDirectory(prefix="snap-") as temp_dir:
            wav_path = Path(temp_dir) / LOCAL_AUDIO_FILENAME
            self._save_wav(wav_path, frames)
            result = self._run_bin(wav_path)
...
class TranscriptionClient(TranscriptionRuntime):
    """Backward-compatible alias for the transcription runtime."""
